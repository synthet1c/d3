...
export default function chart(target, size, data) {
	...
	function filterData(color, data) {
		switch (color) {}
	}
	...
	function resize(e) {
		...
		if (isMobile()) {
			...
			if (!currentColor) {
				...
				update(data, 500, 'mobile')
			}
		}
		else {
			initDesktop(currentData, 500, 'desktop')
			update(data, 500, 'desktop')
		}
	}
	...
	function initMobile(data, speed) {
		...
		Wrapper.attr('class', `chart mobile ${currentColor}`)
	}
	...
	function update(data, speed = 500, size = 'mobile') {
        ...
        .style('height', d => (yScale.bandwidth() + settings.bar.padding_bars) + 'px')
				.text(d => d.name)
		// Update
		Label
			.attr('class', 'chart__label')
		...
		switch (getPosition(data)) {
			...
			case 'all':
				...
				Labels.selectAll('.chart__label')
					.transition()
					.duration(speed)
					.style('left', d => d.value < 0 ? (xScale(0) + 30 + settings.content.padding.left) + 'px' : xScale(0) - 300 - 10 + settings.content.padding.left + 'px')
		...
		/*
		Footer.style('transform', translate(
			px(0),
			px(currentHeight - settings.footer.height * 2)
		))
		*/
	}
		...
		if (isMobile()) {
			switch (d.colorName) {
				case 'purple':
					...
					break
				case 'teal':
					update(allData.filter(d => d.value >= -5 && d.value <= 5), 500, 'mobile')
					break
				case 'blue':
					update(allData.filter(d => d.value > 5), 500, 'mobile')
					break
