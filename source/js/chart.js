...
export default function chart(target, size, data) {
	...
	const mobileResize = debounce(function mobileResize(e) {
		setWidth()
	...
	function setHeight() {
      ...
      + settings.footer.height
		...
		return (
			currentHeight = (currentData.length * yScale.bandwidth()) + chartHeight
		)
	}
	...
	function initMobile(data, speed) {
		...
		Content.attr('width', currentWidth)
		...
		settings.amount = {}
		settings.amount.xpadding = 10
		settings.amount.ymargin = 20


		let _aniSpeed = 500

		var newGroup = Content.selectAll(".chart__bar")
			.data(currentData);
		...
		switch (getPosition(data)) {
			...
			case 'start':
				Content.attr('transform', translate(settings.content.padding.right - 20, settings.header.height))
				// rescale for text
				xScale = d3
					.scaleLinear()
				...
				barEntry.select(".chart__bar--container .chart__bar--bar")
					...
					.transition()
					.duration(_aniSpeed)
					.attr("width", d => currentWidth - 40 - d3.max([xScale(d.value), 0]))
					.attr('x', d => d3.max([xScale(d.value), 0]))
			...
			case 'middle':
				...
				barEntry
					...
					.attr("transform", d =>
						d.value >= 0
							? translate(
									settings.amount.xpadding + d3.max([xScale(d.value), 0]),
									settings.amount.ymargin
								)
							: translate(
								Math.abs(settings.amount.xpadding + xScale(0)),
								settings.amount.ymargin
							)
					)
			...
			case 'end':
				...
				barEntry
					...
					.transition()
					.duration(_aniSpeed)
					.attr("transform", d => "translate(" + (settings.amount.xpadding + d3.max([xScale(d.value), 0])) + ", " + settings.amount.ymargin + ")");
		...
		/*
		barGroup.selectAll('.chart__bar--value')
		...
		barGroup
			...
			.attr('height', yScale.bandwidth() - settings.bar.padding_bars)
		...
		barGroup
			...
			.append('text')
				...
				.text(d => d.value + '%')
				// .attr('x', d => xScale(Math.abs(d.value)))
				.attr('x', d => Math.abs(xScale(d.value) - xScale(0)))
				.attr('dx', d => 0)
		...
		const barGroupEnter = barGroup
			...
			const barGroupEnterRect = barGroupEnter
				.append('rect')
					.attr('class', d => `chart__bar chart__bar--${setColor(d)}`)
					.attr('transform', (d, i) => `translate(0, ${i * yScale.bandwidth()})`)
					.attr('y', d => yScale(d.value))
		...
		const bar = d3.selectAll('.chart__bar')

		// set the width
		switch (getPosition(data)) {
			case 'start':
				bar.attr('width', d => xScale(Math.abs(d.value)))
				break;

			case 'middle':
				d3.selectAll('.chart__bar')
					.attr('width', d => Math.abs(xScale(d.value) - xScale(0)))
					.attr('x', d => xScale(Math.min(0, d.value)))
				d3.selectAll('.chart__bar--value')
					.attr('x', d => Math.abs(xScale(d.value) - xScale(0)))
				d3.selectAll('.chart__bar--bar')
					.attr('width', d => Math.max(3, xScale(d.value)))
				break;

			case 'end':
				d3.selectAll('.chart__bar--bar')
					.attr('width', d => Math.max(3, xScale(Math.abs(d.value))))
					.attr('x', 0)
					.attr('fill', 'red')

				d3.selectAll('.chart__bar--value')
					.attr('x', d => Math.max(3, xScale(Math.abs(d.value))))
					.text(d => d.value + '%')
					.attr('fill', 'green')
				break;
		}
	*/
		...
		Footer.attr('height', currentHeight)
